...
static void fenster_draw_rect(id v, SEL s, CGRect r) {
    ...
    CGImageRef img = CGImageCreate(f->width, f->height, 8, 32, f->width * 4, space,
                                   kCGImageAlphaNoneSkipFirst | kCGBitmapByteOrder32Little, provider, NULL, false,
...
FENSTER_API int fenster_open(struct fenster *f) {
    ...
    f->wnd = msg4(id, msg(id, cls("NSWindow"), "alloc"), "initWithContentRect:styleMask:backing:defer:", CGRect,
...
FENSTER_API int fenster_loop(struct fenster *f) {
    ...
    id ev = msg4(id, NSApp, "nextEventMatchingMask:untilDate:inMode:dequeue:", NSUInteger, NSUIntegerMax, id, NULL, id,
...
static LRESULT CALLBACK fenster_wndproc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_PAINT: {
        ...
        SetDIBitsToDevice(memdc, 0, 0, f->width, f->height, 0, 0, 0, f->height, f->buf, (BITMAPINFO *)&bi,
                          DIB_RGB_COLORS);
    ...
    case WM_KEYDOWN:
    case WM_KEYUP: {
        f->mod = ((GetKeyState(VK_CONTROL) & 0x8000) >> 15) | ((GetKeyState(VK_SHIFT) & 0x8000) >> 14) |
...
FENSTER_API int fenster_open(struct fenster *f) {
    ...
    RegisterClassEx(&wc);
    f->hwnd = CreateWindowEx(WS_EX_CLIENTEDGE, f->title, f->title, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT,
...
#elif defined(USE_SDL)
struct fenster_keymap {
    SDL_Keycode sdl;
    int code;
};

/* clang-format off */
static const struct fenster_keymap FENSTER_KEYMAP[] = {{SDLK_BACKSPACE, 8}, {SDLK_DELETE, 127}, {SDLK_DOWN, 18},  {SDLK_END, 5}, {SDLK_ESCAPE, 27},   {SDLK_HOME, 2}, {SDLK_INSERT, 26},   {SDLK_LEFT, 20}, {SDLK_PAGEUP, 3}, {SDLK_PAGEDOWN, 4}, {SDLK_RETURN, 10},   {SDLK_RIGHT, 19}, {SDLK_TAB, 9}, {SDLK_UP, 17}, {SDLK_APOSTROPHE, 39}, {SDLK_BACKSLASH, 92}, {SDLK_LEFTBRACKET, 91}, {SDLK_RIGHTBRACKET, 93}, {SDLK_COMMA, 44}, {SDLK_EQUALS, 61}, {SDLK_GRAVE, 96},   {SDLK_MINUS, 45}, {SDLK_PERIOD, 46},   {SDLK_SEMICOLON, 59}, {SDLK_SLASH, 47}, {SDLK_SPACE, 32}, {SDLK_A, 65},  {SDLK_B, 66}, {SDLK_C, 67},  {SDLK_D, 68}, {SDLK_E, 69},  {SDLK_F, 70}, {SDLK_G, 71},  {SDLK_H, 72}, {SDLK_I, 73},  {SDLK_J, 74}, {SDLK_K, 75},  {SDLK_L, 76}, {SDLK_M, 77},  {SDLK_N, 78}, {SDLK_O, 79},  {SDLK_P, 80}, {SDLK_Q, 81},  {SDLK_R, 82}, {SDLK_S, 83},  {SDLK_T, 84}, {SDLK_U, 85},  {SDLK_V, 86}, {SDLK_W, 87},  {SDLK_X, 88}, {SDLK_Y, 89},  {SDLK_Z, 90}, {SDLK_0, 48},  {SDLK_1, 49}, {SDLK_2, 50},  {SDLK_3, 51},   {SDLK_4, 52},  {SDLK_5, 53}, {SDLK_6, 54},  {SDLK_7, 55}, {SDLK_8, 56},  {SDLK_9, 57}};
/* clang-format on */
#define FENSTER_KEYMAP_COUNT (sizeof(FENSTER_KEYMAP) / sizeof(FENSTER_KEYMAP[0]))

FENSTER_API int fenster_open(struct fenster *f) {
    if (!SDL_Init(SDL_INIT_VIDEO | SDL_INIT_EVENTS))
        goto leave;

    f->win = SDL_CreateWindow(f->title ? f->title : "fenster", f->width, f->height, SDL_WINDOW_OPENGL);
    if (!f->win)
        goto quit;

    f->ren = SDL_CreateRenderer(f->win, NULL);
    if (!f->ren)
        goto destroy_window;

    if (!SDL_RenderClear(f->ren))
        goto destroy_renderer;

    SDL_RenderPresent(f->ren);

    f->tex = SDL_CreateTexture(f->ren, SDL_PIXELFORMAT_XRGB8888, SDL_TEXTUREACCESS_TARGET, f->width, f->height);
    if (!f->tex)
        goto destroy_renderer;

    if (!SDL_ShowWindow(f->win))
...
FENSTER_API int fenster_loop(struct fenster *f) {
    ...
    SDL_UpdateTexture(f->tex, NULL, f->buf, f->width * sizeof(uint32_t));

    if (!SDL_RenderClear(f->ren))
        return -1;

    if (!SDL_RenderTexture(f->ren, f->tex, NULL, NULL))
        return -1;
    ...
    while (SDL_PollEvent(&ev)) {
        ...
        case SDL_EVENT_MOUSE_BUTTON_DOWN:
            if (ev.button.button == SDL_BUTTON_LEFT)
                f->mouse |= SDL_BUTTON_LMASK;
            else if (ev.button.button == SDL_BUTTON_MIDDLE)
                f->mouse |= SDL_BUTTON_MMASK;
            else if (ev.button.button == SDL_BUTTON_RIGHT)
                f->mouse |= SDL_BUTTON_RMASK;
            break;
        ...
        case SDL_EVENT_MOUSE_WHEEL:
            f->wheel.event = (struct fenster_wheel_event){ev.wheel.x, ev.wheel.y};
            f->wheel.time = fenster_time();
            break;
        ...
        case SDL_EVENT_KEY_UP: {
            ...
            for (size_t i = 0; i < FENSTER_KEYMAP_COUNT; ++i) {
                if (FENSTER_KEYMAP[i].sdl == k) {
                    int code = FENSTER_KEYMAP[i].code;
            ...
            f->mod = (!!(km & SDL_KMOD_CTRL)) | ((!!(km & SDL_KMOD_SHIFT)) << 1) | ((!!(km & SDL_KMOD_ALT)) << 2) |
                     ((!!(km & SDL_KMOD_GUI)) << 3);
        } break;
        case SDL_EVENT_QUIT:
            return 1;
    ...
    if (fenster_time() - f->wheel.time > 0x80)
        f->wheel.event = (struct fenster_wheel_event){0, 0};
...
FENSTER_API int fenster_loop(struct fenster *f) {
    ...
    while (XPending(f->dpy)) {
        ...
        case KeyRelease: {
            ...
            f->mod =
                (!!(m & ControlMask)) | (!!(m & ShiftMask) << 1) | (!!(m & Mod1Mask) << 2) | (!!(m & Mod4Mask) << 3);
        } break;
        }
    }
    return 0;
...
class Fenster {
  ...
  public:
    Fenster(const int w, const int h, const char *title) : f{.title = title, .width = w, .height = h} {
