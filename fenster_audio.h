...
FENSTER_API void fenster_audio_close(struct fenster_audio *fa) { waveOutClose(fa->wo); }
#elif defined(__linux__)
extern int snd_pcm_open(void **, const char *, int, int);
extern int snd_pcm_set_params(void *, int, int, int, int, int, int);
extern int snd_pcm_avail(void *);
extern int snd_pcm_writei(void *, const void *, unsigned long);
