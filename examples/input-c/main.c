    ...
    while (fenster_loop(&f) == 0) {
        ...
        *p = '\0';
        ...
        if (has_keys) {
            ...
            if (f.mod & 4)
                fenster_text(&f, 88, 40, "Alt", 4, 0xffffff);
        }
#if USE_SDL
        if (f.mouse) {
            if (f.mouse & SDL_BUTTON_LMASK)
                fenster_text(&f, 8, 120, "LMB", 4, 0xffffff);
            if (f.mouse & SDL_BUTTON_MMASK)
                fenster_text(&f, 68, 120, "MMB", 4, 0xffffff);
            if (f.mouse & SDL_BUTTON_RMASK)
                fenster_text(&f, 128, 120, "RMB", 4, 0xffffff);
        }

        if (f.wheel.event.x)
            fenster_text(&f, 8, 160, f.wheel.event.x > 0 ? "+X" : "-X", 4, 0xffffff);

        if (f.wheel.event.y)
            fenster_text(&f, 8, 200, f.wheel.event.y > 0 ? "+Y" : "-Y", 4, 0xffffff);
#endif
        if (f.keys[27]) {
            break;
        }
